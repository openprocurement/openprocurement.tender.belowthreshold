...
class TenderResourceTest(BaseTenderWebTest):
    ...
    def test_docs_multi_lots_tutorial(self):
        ...
        response = self.app.patch_json(
            '/tenders/{}/auction/{}?acc_token={}'.format(self.tender_id, first_lot['id'], owner_token),
            {'data': patch_data})
        self.assertEqual(response.status, '200 OK')

        response = self.app.patch_json(
            '/tenders/{}/auction/{}?acc_token={}'.format(self.tender_id, second_lot['id'], owner_token),
            {'data': patch_data})
        self.assertEqual(response.status, '200 OK')
        self.app.authorization = ('Basic', ('broker', ''))
        ...
        # Get contracts
        with open('docs/source/tutorial/all_pending_contracts.http', 'w') as self.app.file_obj:
            response = self.app.get('/tenders/{}/contracts'.format(self.tender_id))
            self.main_contract, self.additional_contract = response.json['data']

        #### Merge contracts
        with open('docs/source/tutorial/merge_contracts.http', 'w') as self.app.file_obj:
            response = self.app.patch_json('/tenders/{}/contracts/{}?acc_token={}'.format(
        ...
        with open('docs/source/tutorial/additional_contract.http', 'w') as self.app.file_obj:
        ...
        with open('docs/source/tutorial/cancel_main_contract.http', 'w') as self.app.file_obj:
            response = self.app.patch_json('/tenders/{}/awards/{}?acc_token={}'.format(
        ...
        with open('docs/source/tutorial/contract_after_cancel_main_award.http', 'w') as self.app.file_obj:
