...
def patch_tender_award_unsuccessful(self):
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(self.tender_id, award['id'], bid_token), {'data': {
    ...
    response = self.app.post_json('{}/complaints?acc_token={}'.format(new_award_location[-81:], bid_token), {'data': {
        'title': 'complaint title',
        'description': 'complaint description',
        'author': test_organization
    }})
    ...
    response = self.app.patch_json('{}?acc_token={}'.format(new_award_location[-81:], self.tender_token), {"data": {"status": "unsuccessful"}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertIn('Location', response.headers)
    new_award_location = response.headers['Location']
...
def patch_tender_lot_award_unsuccessful(self):
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(self.tender_id, award['id'], token), {'data': {
        'title': 'complaint title',
        'description': 'complaint description',
        'author': test_organization,
        'status': 'claim'
    ...
    response = self.app.post_json('{}/complaints?acc_token={}'.format(new_award_location[-81:], token), {'data': {
        'title': 'complaint title',
        'description': 'complaint description',
        'author': test_organization
    }})
...
def create_tender_award_complaint_invalid(self):
    ...
    response = self.app.post_json('/tenders/some_id/awards/some_id/complaints?acc_token={}'.format(token), {
...
def create_tender_award_complaint(self):
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(
        self.tender_id, self.award_id, token), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_organization, 'status': 'claim'}})
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(
        self.tender_id, self.award_id, token), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_organization}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
...
def patch_tender_award_complaint(self):
    token = self.initial_bids_tokens.values()[0]
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(
        self.tender_id, self.award_id, token), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_organization}})
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    complaint = response.json['data']
    ...
    response = self.app.patch_json('/tenders/{}/awards/{}/complaints/some_id?acc_token={}'.format(self.tender_id, self.award_id, owner_token), {"data": {"status": "resolved", "resolution": "resolution text"}}, status=404)
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/tenders/some_id/awards/some_id/complaints/some_id?acc_token={}'.format(owner_token), {"data": {"status": "resolved", "resolution": "resolution text"}}, status=404)
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/tenders/{}/awards/some_id/complaints/some_id?acc_token={}'.format(self.tender_id, owner_token),
...
def review_tender_award_complaint(self):
    ...
    complaints = []
    for i in range(3):
        response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(self.tender_id, self.award_id, bid_token), {'data': {
            'title': 'complaint title',
            'description': 'complaint description',
            'author': test_organization,
            'status': 'claim'
...
def get_tender_award_complaints(self):
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(
        self.tender_id, self.award_id, bid_token), {'data': {'title': 'complaint title', 'description': 'complaint description', 'author': test_organization}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
...
def patch_tender_lot_award_complaint(self):
    ...
    response = self.app.patch_json('/tenders/some_id/awards/some_id/complaints/some_id?acc_token={}'.format(owner_token),
                                   {"data": {"status": "resolved", "resolution": "resolution text"}}, status=404)
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
...
def not_found(self):
    response = self.app.post('/tenders/some_id/awards/some_id/complaints/some_id/documents?acc_token={}'.format(self.complaint_owner_token), status=404, upload_files=[
    ...
    response = self.app.post('/tenders/{}/awards/some_id/complaints/some_id/documents?acc_token={}'.format(self.tender_id, self.complaint_owner_token),
    ...
    response = self.app.post('/tenders/{}/awards/{}/complaints/some_id/documents?acc_token={}'.format(self.tender_id, self.award_id, self.complaint_owner_token), status=404, upload_files=[('file', 'name.doc', 'content')])
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.put('/tenders/some_id/awards/some_id/complaints/some_id/documents/some_id?acc_token={}'.format(self.complaint_owner_token), status=404,
    ...
    response = self.app.put('/tenders/{}/awards/some_id/complaints/some_id/documents/some_id?acc_token={}'.format(self.tender_id, self.complaint_owner_token), status=404,
                            upload_files=[('file', 'name.doc', 'content2')])
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    ...
    response = self.app.put('/tenders/{}/awards/{}/complaints/some_id/documents/some_id?acc_token={}'.format(self.tender_id, self.award_id, self.complaint_owner_token), status=404, upload_files=[
    ...
    response = self.app.put('/tenders/{}/awards/{}/complaints/{}/documents/some_id?acc_token={}'.format(
        self.tender_id, self.award_id, self.complaint_id, self.complaint_owner_token), status=404, upload_files=[('file', 'name.doc', 'content2')])
    self.assertEqual(response.status, '404 Not Found')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
...
def create_tender_award_complaint_document(self):
    ...
    response = self.app.post('/tenders/{}/awards/{}/complaints/{}/documents?acc_token={}'.format(
        self.tender_id, self.award_id, self.complaint_id, self.complaint_owner_token), upload_files=[('file', 'name.doc', 'content')], status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
...
def put_tender_award_complaint_document(self):
    ...
    response = self.app.put('/tenders/{}/awards/{}/complaints/{}/documents/{}?acc_token={}'.format(self.tender_id, self.award_id, self.complaint_id, doc_id, self.complaint_owner_token),
                            status=404,
